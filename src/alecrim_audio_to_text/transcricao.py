...
EnvVariable = namedtuple(
    typename="EnvVariable", field_names=["NOME_PROJETO", "SPEECH_KEY", "SPEECH_REGION"]
)
...
def audio_to_text(
    ...
    env_variables: EnvVariable = EnvVariable("", "", ""),
) -> None:
    """
    ...
    # Diretórios
    DIR_PROJETO = get_path_project(nome_projeto=env_variables.NOME_PROJETO)
    ...
    speech_config = speechsdk.SpeechConfig(
        subscription=env_variables.SPEECH_KEY,
        region=env_variables.SPEECH_REGION,
    )
    ...
    speech_recognizer.session_started.connect(
        lambda evt: print("Sessão iniciada: {}".format(evt))
    )
    speech_recognizer.session_stopped.connect(
        lambda evt: print("Sessão encerrada: {}".format(evt))
    )
